...
export class ErrorComponent implements OnInit {
  ...
  @Input() key: any;
  showError = [""];
  static errorCheck: EventEmitter<FormGroup> = new EventEmitter();
  ngOnInit(): void {
    ErrorComponent.errorCheck.subscribe((data: FormGroup) => {
      if(data==this.userForm)
      this.validateError(data);
    });
  }
  validateError(form: FormGroup) {
    this.showError = [];
